...
export default function Dashboard() {
  ...
  const load = () => Promise.all([
    ...
    api.get("/api/assets").then(r => setAssets(r.data || [])),
    api.get("/api/assets", { params: { isBookable: true } }).then(r => setBookableAssets(r.data || [])),
    api.get("/api/transfers").then(r => setTransfers(r.data || [])),
  ]).catch(() => setMsg("API error — is the server running?"));
  ...
  const upcomingBookings = books.filter(b => b.status === "Upcoming").length;
  const pendingMaintenance = maint.filter(m => m.status === "Pending").length;
  const pendingTransfers = transfers.filter(t => t.status === "Requested").length;
  const upcomingReturns = active.filter(a => a.expectedReturnDate && new Date(a.expectedReturnDate) >= new Date()).length;
    ...
    <div className="space-y-5">
      {msg && <div className="p-3 bg-red-50 border border-red-200 text-red-700 rounded">{msg} <button onClick={() => setMsg("")} className="ml-2 font-bold">×</button></div>}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          {[["Assets Available", assets.filter(a => a.status === "Available").length, false], ["Assets Allocated", active.length, false], ["Maintenance Today", maint.filter(m => m.status === "Approved" || m.status === "InProgress").length, false], ["Active Bookings", upcomingBookings, false], ["Pending Transfers", pendingTransfers, pendingTransfers > 0], ["Upcoming Returns", upcomingReturns, upcomingReturns > 0]].map(([l, v, warn]) => (
        ...
        <div className="p-3 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
          {overdue > 0 ? `${overdue} overdue return${overdue > 1 ? "s" : ""} require attention.` : "No overdue returns at the moment."}
        </div>
        ...
        <div className="grid grid-cols-3 gap-5 items-start">
          <div className="col-span-3 grid grid-cols-3 gap-3">
            {[
              ...
              <button key={action.label} onClick={() => nav(action.path)} className="px-3 py-2 bg-zinc-900 text-white rounded text-xs font-semibold hover:bg-zinc-700 text-left">
                {action.label}
              </button>
            ))}
          </div>
          <div className="col-span-2 space-y-4">
            {[
              { title: user.role === "Employee" ? "My Assets" : "Active Allocations", rows: active, render: a => <><span className="font-medium">{a.asset?.name}</span><span className="ml-2 font-mono text-zinc-400">{a.asset?.assetTag}</span>{a.expectedReturnDate && <span className={`ml-2 ${new Date(a.expectedReturnDate) < new Date() ? "text-red-600 font-semibold" : "text-zinc-400"}`}>Due {new Date(a.expectedReturnDate).toLocaleDateString()}</span>}<button onClick={() => doReturn(a._id)} className="ml-auto px-2 py-0.5 border border-zinc-300 rounded hover:bg-zinc-50">Return</button></> },
              { title: "Upcoming Bookings", rows: books.filter(b => b.status === "Upcoming"), render: b => <><span>{b.resource?.name}</span><span className="ml-2 text-zinc-400">{new Date(b.startTime).toLocaleString([], { dateStyle: "short", timeStyle: "short" })} → {new Date(b.endTime).toLocaleTimeString([], { timeStyle: "short" })}</span><span className="ml-auto font-semibold text-blue-600">{b.status}</span></> },
          ...
          <div className="space-y-4">
            <form onSubmit={doBook} className="bg-white border border-zinc-200 rounded p-4 space-y-2">
              ...
              <select required className={inp} value={bf.resource} onChange={e => setBf({ ...bf, resource: e.target.value })}><option value="">Select resource...</option>{bookableAssets.map(a => <option key={a._id} value={a._id}>{a.name} ({a.assetTag})</option>)}</select>
            ...
            <form onSubmit={doMaint} className="bg-white border border-zinc-200 rounded p-4 space-y-2">
              ...
              <select required className={inp} value={mf.asset} onChange={e => setMf({ ...mf, asset: e.target.value })}><option value="">Select asset...</option>{assets.map(a => <option key={a._id} value={a._id}>{a.name} ({a.assetTag})</option>)}</select>
